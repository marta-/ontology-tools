 ...
 */
package edu.toronto.cs.ontools.utils.maps;
...
public abstract class AbstractNumericValueMap<K, N extends Number> extends
		TreeMap<K, N> implements NumericValueMap<K, N> {
	...
	public List<K> sort(final boolean descending) {
		...
		Arrays.sort(sortedKeys, new Comparator<K>() {
			public int compare(K a, K b) {
				...
				try {
					return ((((Comparable<N>) safeGet(a)).compareTo(safeGet(b)) > 0) && descending) ? -1
							: 1;
